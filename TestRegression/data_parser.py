...
def clean_communities(num_sens):
    ...
    df = pd.read_csv('TestRegression/data/communities.csv')
    df = df.fillna(0)
...
def clean_adult_full():
    ...
    """
    df = pd.read_csv("TestRegression/data/adult_full.csv", )
    df = df.dropna()
    ## Determine if rows or columns which contain missing values are removed.
    df = df.replace({'?':np.nan}).dropna()
    ## replace any occurrance of "?" with np.nan
    df["income"] = df["income"].map({'<=50K': 0, '>50K': 1})
    ## not regression problem???
    y = df["income"]
    df = df.drop("income", 1)
    ## drop the column of income

    # hot code categorical variables
    df = one_hot_code(df)
    ## each categories of n different labels are represented by n columns.
    log_numeric_features(df)
    ## converting every non-binary feature to their log(1+x)
    a = df['sex']
...
def majority_pop(a):
    ...
    ## idxmax returns the label of maximum value
    return maj
...
def clean_lawschool_full():
    ...
    """
    df = pd.read_csv('TestRegression/data/lawschool.csv')
    df = df.dropna()
    # remove y from df
    y = df['ugpa']
    y = y / 4
