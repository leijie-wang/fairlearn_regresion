...
def loss_vec(tp, y, result_weights, loss='square'):
    ...
    loss_list = [loss.eval(tp.iloc[:,i],y) for i in range(num_h)]
    ...
    # elif isinstance(loss,LogisticLoss):
...
def extract_pred(X, pred_aug, Theta):
    ...
    for i in range(n):
        ...
        #theta_index = max(0, (sum([p_vec[i] for p_vec in pred_list]) - 1))
...
def log_loss_vec(y_true, y_pred, eps=1e-15):
    ...
    ## this might be for the prevention of underflow.

    # If y_pred is of single dimension, assume y_true to be binary
    # and then check.
    if y_pred.ndim == 1: #y_pred is a simple vertical vector like [0,1,2,..]
        y_pred = y_pred[:, np.newaxis]
        ## becomes lists of list like [[0],[1],[2],...]
    if y_pred.shape[1] == 1:
        y_pred = np.append(1 - y_pred, y_pred, axis=1)
        ## becomes [[0,1],[1,0],..]
    ## Isn't this required to be true???

    # Renormalize
    y_pred /= y_pred.sum(axis=1)[:, np.newaxis] 
    ## sum for each row, which results in a vector of shape [n,1].
    ## seems opposite to the meaning of the axis of the concat or append function
    ## But it seems that the sum for each row is necessarily one, 
    ## so this statement does not change the value of y_pred

    trans_label = pd.concat([1-y_true, y_true], axis=1)
    loss = -(trans_label * np.log(y_pred)).sum(axis=1)
    ## element-wise multiplication. 
    #logistic loss is -y_true * ln(y_pred) - (1-y_true) * ln(1 - y_pred)
    return loss
