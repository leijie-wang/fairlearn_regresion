import pandas as pd
import numpy as np
from fairlearn.reductions._moments.moment import LossMoment,ClassificationMoment,_ALL, _LABEL
...
class WeightedErrorRate(LossMoment):
    ...
    def gamma(self, predictor):
        if isinstance(self.reduction_loss,SquareLoss):
            def h(X): return 1*( predictor(X.drop(['theta'], axis=1)) - X['theta'] >= 0)
        else: #cases of Logistic Loss.
